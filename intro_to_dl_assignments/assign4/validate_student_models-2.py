...
def try_with_random_data(session, inputs, predict_op):
  ...
  # Get a random batch.
  batch = np.random.rand(batch_size, 32, 32, 3)
  print("(Batch of shape (%d, 32, 32, 3)" % (batch_size))

  # Put it through the model.
  predictions = session.run(predict_op, feed_dict={inputs: batch})
...
def main():
  ...
  with tf.Session() as session:
    ...
    if args.val_data_file:
      ...
      try_with_random_data(session, inputs, predict_op)
